...
const columns = [
    ...
    {
        name: 'Nombre',
        selector: row => `${row.f_name} ${row.s_name} ${row.f_lastname} ${row.s_lastname}`,
        sortable: true,
        width: '25%',
        wrap: true,
    },
    ...
    {
        name: 'Escuelas',
        selector: row => row.schools.join(', '),
        /* sortable: true */
        /* cell: row => (
			<div>
				{row.schools.map((school, i) => (
					<p key={i}>{school}</p>
				))}
			</div>
		), */
...
export default function AdminUsersTable() {
    ...
    const filteredItems = users.filter(item => {
        const matchesName = (item.f_name && item.s_name && item.f_lastname && item.s_lastname)  && 
            (item.f_name.toLowerCase().includes(filterText.toLowerCase())) || (item.f_name.toLowerCase().includes(filterText.toLowerCase())) ||
            (item.s_name.toLowerCase().includes(filterText.toLowerCase())) || (item.s_name.toLowerCase().includes(filterText.toLowerCase())) ||
            (item.f_lastname.toLowerCase().includes(filterText.toLowerCase())) || (item.f_lastname.toLowerCase().includes(filterText.toLowerCase())) ||
            (item.s_lastname.toLowerCase().includes(filterText.toLowerCase())) || (item.s_lastname.toLowerCase().includes(filterText.toLowerCase()));
        return matchesName
    })

    useEffect(() => {
        axios.get('https://funval-api.onrender.com/api/v1/users/', {
            headers: {
        ...
        <>
            <div className='flex flex-col mt-5 mx-auto'>
                ...
                <DataTable
                    ...
                    noDataComponent='No hay datos que coincidan con su búsqueda'
                    responsive
                />
            </div>
        </>
